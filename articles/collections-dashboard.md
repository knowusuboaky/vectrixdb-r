...
This article documents the **collections dashboard** launcher and common
runtime options from `collections dashboard/run_dashboard.R`.
...
Passing `--data-path` explicitly keeps the runtime aligned with the
local blob folder.
...
- Confirm `--data-path` points to the same local collection path used by
  ingestion.
- If startup fails, rerun with `--print-only` first to validate resolved
  settings.
